...
let streamerState = {
    ...
    twitchToken: '',      // OAuth токен для отправки сообщений
    overlayOpen: false,
...
// ── TWITCH DIRECT AUTHORIZATION ──────────────────────────
const TWITCH_IRC = 'wss://irc-ws.chat.twitch.tv:443';
// Используем публичный Client ID для демонстрации (замените на свой)
const TWITCH_CLIENT_ID = 'uo6dggojyb8d6soh92zknwmi5ej1q2'; // Получите на https://dev.twitch.tv/console/apps

// Прямая авторизация через Twitch Implicit Flow
function authorizeWithTwitch() {
    if (TWITCH_CLIENT_ID === 'YOUR_TWITCH_CLIENT_ID_HERE') {
        showTwitchSetupInstructions();
        return;
    }

    const redirectUri = encodeURIComponent(window.location.origin);
    const scopes = encodeURIComponent('chat:read chat:edit user:read:chat user:write:chat moderator:read:chatters');
    const state = btoa(Math.random().toString()).slice(0, 20);

    // Сохраняем state для проверки CSRF
    localStorage.setItem('twitch_oauth_state', state);

    const authUrl = `https://id.twitch.tv/oauth2/authorize?response_type=token&client_id=${TWITCH_CLIENT_ID}&redirect_uri=${redirectUri}&scope=${scopes}&state=${state}`;

    showNotification('Перенаправление на авторизацию Twitch...', 'info');
    window.location.href = authUrl;
}

// Показать инструкцию по настройке Client ID
function showTwitchSetupInstructions() {
    const modal = document.getElementById('confirmModal');
    if (!modal) return;

    document.getElementById('modalTitle').textContent = '⚙️ Настройка Twitch приложения';
    document.getElementById('modalMessage').innerHTML = `
        <div style="text-align:left;font-size:13px;line-height:1.5">
            <p><strong>Для прямой авторизации нужно создать Twitch приложение:</strong></p>
            
            <ol style="padding-left:20px;margin:16px 0">
                <li>Перейдите на <a href="https://dev.twitch.tv/console/apps" target="_blank" style="color:var(--accent-primary)">dev.twitch.tv/console/apps</a></li>
                <li>Нажмите "Register Your Application"</li>
                <li>Заполните форму:
                    <ul style="margin-top:4px">
                        <li><strong>Name:</strong> Название вашего приложения</li>
                        <li><strong>OAuth Redirect URLs:</strong> <code style="user-select:all;background:var(--bg-tertiary);padding:2px 4px">${window.location.origin}</code></li>
                        <li><strong>Category:</strong> Website Integration</li>
                    </ul>
                </li>
                <li>Скопируйте Client ID</li>
                <li>В script.js замените TWITCH_CLIENT_ID на ваш Client ID</li>
            </ol>
            
            <div style="background:var(--bg-tertiary);padding:12px;border-radius:8px;margin:16px 0">
                <strong>✨ Преимущества:</strong><br>
                • Официальная авторизация Twitch<br>
                • Безопасность OAuth 2.0<br>
                • Автоматическое получение токена<br>
                • Никаких сторонних сервисов
            </div>
        </div>
    `;
    document.getElementById('modalConfirm').textContent = 'ПОНЯТНО';
...
// Проверка OAuth токена из URL после редиректа
function checkOAuthToken() {
    ...
    if (hash && hash.includes('access_token=')) {
        ...
        if (state === storedState && accessToken) {
            streamerState.twitchToken = accessToken;
            ...
            showNotification('✅ Авторизация успешна! Токен получен и сохранён', 'success');
            ...
            showNotification('❌ Ошибка проверки состояния авторизации', 'error');
            return false;
        }
    }
    return false;
...
function twitchConnect(channel) {
    ...
    ws.onopen = () => {
        ...
        if (streamerState.twitchToken) {
            ws.send('CAP REQ :twitch.tv/tags twitch.tv/commands twitch.tv/membership');
            ws.send(`PASS oauth:${streamerState.twitchToken}`);
            ws.send(`NICK ${streamerState.streamerProfile.displayName || ch}`);
        } else {
    ...
    ws.onmessage = (e) => {
        ...
        if (raw.includes(`JOIN #${ch}`) && streamerState.twitchStatus !== 'connected') {
            ...
            _updateConnectBtn();
            showNotification(`✅ Подключён к #${ch}`, 'success');
            saveStreamerData();
...
async function fetchChannelInfo(channelName) {
    try {
        // Используем публичный API для получения основной информации
        // В реальном приложении здесь был бы настоящий Twitch API с токеном

        // Пока что устанавливаем базовые данные
        ...
        //     streamerState.streamerProfile.displayName = user.display_name;
        //     streamerState.streamerProfile.profileImage = user.profile_image_url;
        //     streamerState.streamerProfile.viewCount = user.view_count;
        // }
...
function sendTwitchChatMessage(text) {
    if (!streamerState.twitchWs || streamerState.twitchStatus !== 'connected') {
        showNotification('Чат не подключён', 'warning');
        return false;
    }
    ...
    try {
        const message = `PRIVMSG #${streamerState.channelName} :${text}`;
        streamerState.twitchWs.send(message);
        return true;
    } catch (error) {
...
function addAllChattersToWheel() {
    ...
    if (added > 0) {
        ...
        if (added === 1) {
            sendTwitchChatMessage(`Добавлен ${addedUsers[0]} в колесо подписчиков!`);
        } else {
            sendTwitchChatMessage(`Добавлены ${added} участников в колесо подписчиков!`);
        }
    } else {
...
function addSubToWheelFromChat(user) {
    if (!streamerState.subWheelList.includes(user)) {
        ...
        // Отправляем сообщение в чат
        sendTwitchChatMessage(`${user} добавлен в колесо подписчиков!`);
    } else {
...
async function saveStreamerData() {
    if (!streamerDB) {
        ...
        const dataToSave = {
            channelName: streamerState.channelName,
            twitchToken: streamerState.twitchToken,
    ...
    if (!streamerDB) {
        ...
        if (saved) {
            try {
                ...
                streamerState.twitchToken = data.twitchToken || '';
...
function updateTwitchToken(value) {
    // Убираем префикс oauth: если пользователь его включил
    let cleanToken = value.trim();
    if (cleanToken.startsWith('oauth:')) {
        cleanToken = cleanToken.substring(6);
    }

    streamerState.twitchToken = cleanToken;
    localStorage.setItem('twitch_access_token', cleanToken);
    saveStreamerData();

    // Валидируем токен если он введён и достаточно длинный
    if (cleanToken && cleanToken.length > 10) {
        setTimeout(() => validateTwitchToken(), 500); // Небольшая задержка для UX
    }
}

// Отключение авторизации Twitch
function disconnectTwitchAuth() {
    showConfirmModal(
        '🔐 Отключить авторизацию',
        'Это удалит токен доступа. Вы сможете только читать чат, но не отправлять сообщения.',
        'ОТКЛЮЧИТЬ',
        'ОТМЕНА',
        () => {
            streamerState.twitchToken = '';
            localStorage.removeItem('twitch_access_token');
            saveStreamerData();

            showNotification('🔓 Авторизация отключена', 'info');
...
function init() {
    ...
    // Проверяем OAuth токен из URL после редиректа от Twitch
    const foundToken = checkOAuthToken();

    // Проверяем сохранённый токен из localStorage если не было редиректа
    if (!foundToken) {
        const savedToken = localStorage.getItem('twitch_access_token');
        if (savedToken) {
            streamerState.twitchToken = savedToken;
...
function showPopupResult(game, player, task) {
    ...
    if (streamerState.twitchStatus === 'connected') {
        ...
        const resultMessage = `🎰 Результат: ${game} | ${playerName} | ${task}`;
        setTimeout(() => sendTwitchChatMessage(resultMessage), 800);
    }
        ...
        <div class="streamer-tools-grid">
            ...
            : streamerState.twitchStatus === 'connecting' ? '⏳ Подключение...'
                ...
                <div class="channel-input-group" style="margin-bottom:8px">
                    ${streamerState.twitchToken
            ? `<div style="display:flex;align-items:center;gap:8px;padding:8px;background:var(--accent-success-bg);border:1px solid var(--accent-success);border-radius:6px">
                            <span style="color:var(--accent-success);font-size:11px">🔑 Авторизован как ${streamerState.streamerProfile.displayName || 'пользователь'}</span>
                            <button onclick="disconnectTwitchAuth()" class="cyber-btn danger-btn" style="padding:4px 8px;font-size:10px">Отключить</button>
                          </div>`
            : `<button onclick="authorizeWithTwitch()" class="cyber-btn add-btn" style="width:100%">🔐 Войти через Twitch</button>`
        }
                </div>
                <div style="font-size:10px;color:var(--text-muted);margin-bottom:8px">
                    ${streamerState.twitchToken
            ? '✅ Полный доступ — можно отправлять сообщения в чат'
            : '⚠️ Для отправки сообщений нужна авторизация через Twitch'}
                </div>
            ...
            <div class="streamer-tool-card">
                ...
                <div class="input-group" style="margin-top:8px">
                    <input type="text" id="chatMsgInput" placeholder="Своё сообщение в чат..." onkeypress="if(event.key==='Enter')sendChatMsg()">
                    <button onclick="sendChatMsg()" class="cyber-btn add-btn">Отправить</button>
                </div>` : `
                <div style="margin-top:8px;padding:8px;background:var(--bg-tertiary);border-radius:var(--radius-sm);font-size:11px;color:var(--text-muted);text-align:center">
                    🔑 Добавьте OAuth токен для отправки сообщений
                </div>`}
...
function startVote() {
    ...
    const isRealChat = streamerState.twitchStatus === 'connected';
    if (!isRealChat) {
        return showNotification('Подключите Twitch чат для голосования', 'warning');
    }
    ...
    const voteMessage = `🗳️ ГОЛОСОВАНИЕ: ${gameOptions.map((o, i) => `${i + 1}️⃣ ${o}`).join(' | ')} (${streamerState.voteDuration}с)`;
    sendTwitchChatMessage(voteMessage);
...
function sendChatMsg() {
    ...
    // Проверяем наличие токена для отправки в реальный чат
    if (streamerState.twitchStatus === 'connected') {
        if (streamerState.twitchToken) {
            // Отправляем в реальный чат
            if (sendTwitchChatMessage(text)) {
                showNotification('✅ Сообщение отправлено в чат', 'success');
            }
        }
    }

    // Добавляем собственное сообщение как превью в локальный чат
...
// Отправка команд в чат (список команд)
function sendCommandsList() {
    if (streamerState.twitchStatus !== 'connected') {
        showNotification('Сначала подключите чат', 'warning');
        return;
    }
    ...
    const commands = [
        ...
        '!clearwheel - очистить колесо подписчиков',
        '',
        '👥 Команды для зрителей:',
        '!join или !addme - добавиться в колесо',
        '!commands или !help - показать команды'
    ];

    // Отправляем команды строками в чат
    commands.forEach((cmd, idx) => {
        if (cmd) {
            setTimeout(() => {
                sendTwitchChatMessage(cmd);
            }, idx * 300); // Задержка между сообщениями для избежания ratelimit
        }
    });
    ...
    showNotification('📝 Список команд отправлен в чат', 'info');
}
...
function quickSpin() {
    if (streamerState.autoSpin) {
        ...
        startSpin();
        sendTwitchChatMessage('🎰 Запущена рулетка! Спин начался!');
    } else {
        // Обычный переход на вкладку рулетки
        switchTab('roulette');
        setTimeout(() => {
            startSpin();
            sendTwitchChatMessage('🎰 Запущена рулетка!');
        }, 400);
    }
}
